...
void Ticker::draw() {
  ...
  draw_list->AddText(bid_size_pos, white_32, bid_size_text.c_str()); 
  ...
  draw_list->AddText(bid_price_pos, bid_text_color_32, bid_price_text.c_str());

  // best ask price
  std::string ask_price_text = Utils::formatPrice(level1->best_ask);
  ImVec2 ask_price_pos(pos.x + bar_length/2 + 5, y_pos);
  draw_list->AddText(ask_price_pos, ask_text_color_32, ask_price_text.c_str());
  ...
  draw_list->AddText(ask_size_pos, white_32, ask_size_text.c_str());
  ImGui::EndChild();
